#!/usr/bin/env python3
"""Orchestration manifest CLI validator (Wave 2 #5).

CLI:
    python3 scripts/validate_orchestration_manifest.py <manifest.json>
...
# WB-05: manifests are small configs; bound the read so a giant regular file
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print("usage: validate_orchestration_manifest.py <manifest.json>", file=sys.stderr)
        return EXIT_USAGE
    path = Path(args[0])
    if not path.is_file():
    ...
    try:
        ...
        # WB-01: deeply nested JSON overflows the decoder's recursion limit; this
    ...
    from _orchestration_redaction import check_orchestration_manifest

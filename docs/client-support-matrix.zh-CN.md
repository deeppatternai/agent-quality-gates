...
| WorkBuddy AI | `workbuddy-ai` | 完整支持(full) | AQG skills、rules、`mcp.json`、`settings.json` lifecycle hooks、verify/uninstall/is-installed，全部写入独立根 `~/.workbuddy-ai` | 与 CodeBuddy Agent CLI 能力面等价，但产品身份、根目录与所有权完全独立；不与 `workbuddy` 或 `codebuddy` 共享配置根或卸载范围 | 本地证据：`product.json` 声明 `customUserDataDir=.workbuddy-ai`；主进程为内嵌 CLI 注入 `WORKBUDDY_CONFIG_DIR`/`CODEBUDDY_CONFIG_DIR=~/.workbuddy-ai`；macOS bundle id `com.workbuddy.workbuddy-ai` |

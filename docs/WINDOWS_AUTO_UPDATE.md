...
- SessionStart 触发器在后台检查签名 stable 发布，默认检查间隔为一小时；也可通过 `AQG_NO_UPDATE_CHECK` 关闭。客户端须实际加载并允许执行对应 hooks。
...
- hook 定义或 Codex 校验摘要变化、skill 新增/删除、配置异常时仍返回 `pending`，需要显式升级并协调客户端配置。不会绕过 Codex 对新 hook 定义的信任要求。

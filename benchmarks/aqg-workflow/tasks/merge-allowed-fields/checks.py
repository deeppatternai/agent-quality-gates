"""Held-out scorer for profile mass-assignment prevention."""
from __future__ import annotations

from typing import Callable

_VALID = [
    ({"display_name": "Ada", "timezone": "UTC"}, {"display_name": "Ada", "timezone": "UTC"}),
    ({"display_name": "Grace", "role": "admin"}, {"display_name": "Grace"}),
    ({"timezone": "Asia/Shanghai", "ignored": 1}, {"timezone": "Asia/Shanghai"}),
]
_REJECT = [
    {"display_name": ""}, {"timezone": 3}, {"display_name": "   "},
    {"role": "admin"}, [],
]


def run_checks(fn: Callable[[dict], dict]) -> list[str]:
    failures: list[str] = []
    for value, expected in _VALID:
        try:
            got = fn(value)
        except Exception as exc:  # aqg: top-level boundary
            failures.append(f"valid {value!r} raised {exc!r}")
        else:
            if got != expected:
                failures.append(f"valid {value!r} -> {got!r}, expected {expected!r}")
    for value in _REJECT:
        try:
            got = fn(value)
        except ValueError:
            continue
        except Exception as exc:  # aqg: top-level boundary
            failures.append(f"invalid {value!r} raised {type(exc).__name__}, expected ValueError")
        else:
            failures.append(f"invalid {value!r} accepted -> {got!r}")
    return failures

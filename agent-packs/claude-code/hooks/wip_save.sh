#!/usr/bin/env bash
# AQG WIP save — Claude Code PreCompact hook wrapper.
#
...
# Optional first arg = CLAUDE_PROJECT_DIR; export so wip_save.py can pin
# to the real project dir instead of the hook's runtime cwd.
if [ "$#" -ge 1 ] && [ -n "${1:-}" ]; then
  export AQG_HOOK_PROJECT_DIR="$1"
fi

exec python3 "$AQG_ROOT/scripts/wip_save.py"
